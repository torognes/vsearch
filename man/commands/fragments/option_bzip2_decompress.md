...
: Specify that the *input pipe* is streaming data compressed using
  Huffman coding. See `bzip2(1)` for more details. This option is
  ...
  compressed with bzip2, nor when such a file is redirected to
  standard input: compression is then detected automatically, and a
