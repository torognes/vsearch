...
: Write results to *filename* in a tab-separated uclust-like format
  ...
  (`S`), hits (`H`), and cluster summaries (`C`). Columns are:
    ...
    8. CIGAR alignment string (`H`), or `*` (`S`, `C`); `=` denotes
       strictly identical sequences (with `--cluster_fast`, identical
       ignoring terminal gaps); the dereplication commands always
       write `*` here; see
