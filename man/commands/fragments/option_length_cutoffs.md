...
: Set the range of truncation lengths to use as rows in the output
  ...
  input file. The shortest cutoff must be at least 1 and not larger
  than the longest, the increment must be at least 1, and the list
  may not contain whitespace. The default is `50,*,50`, producing
  rows at 50, 100, 150, and so on up to the longest sequence.
