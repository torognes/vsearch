% vsearch-uchime2_denovo(1) version 2.31.0 | vsearch manual
% Torbjørn Rognes, Tomás Flouri, and Frédéric Mahé
#(./fragments/date.md)

# NAME

vsearch \-\-uchime2_denovo --- detect chimeras *de novo* using the UCHIME2 algorithm
...
except that its default minimum abundance skew (`--abskew`) is set to
16.0 rather than 2.0.
...
Detect chimeras *de novo* using the UCHIME2 algorithm and write
non-chimeric sequences to a file:

```sh
vsearch \
    --uchime2_denovo denoised.fasta \
    --sizein \
    --nonchimeras clean.fasta
```

A typical denoising workflow: dereplicate, denoise, then remove
chimeras with the UCHIME2 algorithm:
