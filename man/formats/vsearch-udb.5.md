% vsearch-udb(5) version 2.32.0 | vsearch file formats
...
masked sequences, which matches what `--usearch_global` would have done
to a fasta database, but not what `--sintax` does (see
